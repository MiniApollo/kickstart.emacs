...
After you complete that, you should start learning Emacs lisp.
...
Here are some notes on what each use-package keyword does.
...
If you want to use vim keybindings I left the following comments in the General Keybindings section as to which lines to uncomment ;; <- evil

If you don't want to interfere with the original keybindings.
You can also try out [[https://github.com/meow-edit/meow][meow]] which is Yet another modal editing for Emacs.
...
If you want to use this. Don't forget to add keybindings for undo-fu and vundo.
...
#+begin_src emacs-lisp
    (use-package doom-themes
      ...
      :config
      (load-theme 'doom-gruvbox t) ; We need to add t to trust this package
...
You can also create a [[https://github.com/theprimeagen/tmux-sessionizer][tmux-sessionizer]] like [[https://github.com/MiniApollo/config/blob/main/emacs/init.org#project-sessionizer][setup]] to easily search your work directories.
It's useful if you don't use standard project structures (or there is none).
#+begin_src emacs-lisp
...
To control how an LSP server is started customize the eglot-server-programs list.
...
After Emacs 31, you only need to manage the sources for external packages, because the built-in packages now manage their own sources.

To explore the current buffer's syntax tree, use the treesit-explore command.
...
#+begin_src emacs-lisp :tangle no
    (use-package emacs
      ...
      ;; External packages are the only things needed here.
      (treesit-language-source-alist
       '((gdscript "https://github.com/PrestonKnopp/tree-sitter-gdscript")
         (vue "https://github.com/ikatyang/tree-sitter-vue")))
      :config
      ;; Example how to remap external mode if it is not remapped by 'treesit-enabled-mode'.
...
If you want a faster and more responsive terminal emulator try out vterm or ghostel.
Ghostel is currently the fastest one I've found. It's nearly as fast as a native terminal.
#+begin_src emacs-lisp
    (use-package eat
      :defer t
