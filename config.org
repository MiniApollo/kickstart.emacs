...
** Use Package setup
I used the default package manager package.el with use-package. Alternative package managers(straight.el, elpaca, etc..) are useful if you want to install packages from github 
(Like if you want a specific not very popular package that is not avalible on elpa or melpa) 
Or want UI (elpaca) and other things if you are interested check out their github or search for more package managers
...
#+begin_src emacs-lisp
    (use-package evil
      ...
      (setq evil-want-keybinding nil) ;; Disable to load evil bindings to other modes (It's not consistent and not good)
      (setq evil-want-C-u-scroll t) ;; Set  ctrl u to scrool up
      (setq evil-want-C-i-jump nil) ;; Disables Ctrl i jump
...
Learder key is what you will presess when want to access your keybindings (Space + . Find file)
...
** Flycheck 
Flycheck is recommended by lsp-mode for better error handling (lsp mode right side error display only works with flycheck)
#+begin_src emacs-lisp
...
The build in (default) is now eglot but it requires to manualy download the lsp clients, doesn't support debuggers, lsp-mode is pretier and has more features,
Eglot is faster and more minimal, but it requires manual setup
*** Setup
#+begin_src emacs-lisp
...
Currently its tedious to use Treesitter because emacs hasn't translated to it yet fully 
*** Lua mode
#+begin_src emacs-lisp
...
*** ORG MODE
Org mode is one of the things that emacs is loved for you will understand if you start using it or read about what it can do (like this document is the source code)
...
By default, the shell is set to 'fish' but could change it to 'bash' or 'zsh'.
#+begin_src emacs-lisp
    (use-package vterm
      :commands (vterm))
...
Don't forget to use all-the-icons-install-fonts
#+begin_src emacs-lisp
...
** WHICH-KEY
Which-key is a helping utility for keycords (which key to press)
#+begin_src emacs-lisp
